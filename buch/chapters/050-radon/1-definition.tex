...
\varphi(\cdot,0)&\colon&\mathbb{R}&\to&\mathbb{C}^*&:&x&\mapsto&\varphi(x,0)
\\
\varphi(0,\cdot)&\colon&\mathbb{R}&\to&\mathbb{C}^*&:&y&\mapsto&\varphi(0,y)
\end{array}
\end{equation*}
...
Wie Abbildung~\ref{buch:gruppen:radon:fig:ebenewelle} zeigt, beschreiben
die Funktionen $\varphi_{kl}(x,y)$ ebene Wellen.
Die Abbildung zeigt nur die Werte des Imaginärteils $\Im \varphi_{kl}(x,y)$,
blau für negative Werte und grün für positive Werte.
...
auch als die Zusammensetzungen
\[
\mathscr{F}
=
...
\]
der Transformationen $\mathscr{F}_y$ und $\mathscr{F}_x$ lesen.
...
\begin{definition}[$n-1$-dimnensionale Sphäre]
Die {\em $n-1$-dimensionale Sphäre} ist die Teilmenge
\index{Sphäre, $n-1$-dimensional}%
\[
S^{n-1}
=
\{
...
\begin{definition}[Hyperebene $E(s,\omega)$]
\index{Esomega@$E(s,\omega)$}%
...
Der Gram-Schmidt-Prozess kann dazu verwendet werden.
...
\,ds
\]
für Vektoren $\xi\in\mathbb{R}^{n-1}$ zu schreiben.
...
\begin{definition}[Radon-Transformation]
...
\begin{satz}[Radon- und Fourier-Transformation]
...
Insbesondere ist die Radon-Transformation umkehrbar.
\end{satz}
...
Abbildung~\ref{buch:gruppen:radon:definition:fig:radonft} illustriert
dies im Falle $n=2$.
\end{proof}

\begin{satz}[Radon-Umkehrformel]
Ist $u\colon \mathbb{R}^n\to\mathbb{C}$ eine integrierbare Funktion
...
Radon-Transformation nicht besonders stabil ist.
Dies lässt sich auch aus
Abbildung~\ref{buch:gruppen:radon:definition:fig:radonft}
erahnen.
Die Transformation auf Polarkoordinaten quetscht viel wichtige Information
aus der Radon-Transformation in der Nähe des Nullpunkts zusammen.
Wegen der beschränkten Auflösung der diskreten Fourier-Transformation
geht daher ein Teil der für die Rekonstruktion nötigen Information
verloren.
Für Punkte weit weg vom Nullpunkt erhält die Darstellung unten rechts
nicht genügen Information.
