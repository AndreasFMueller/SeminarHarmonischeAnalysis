...
In der Grafik XXX sind zwei unterschiedliche Trajektorien im mit dem Intervall  t = [0,3] eingezeichnet. Einer mit dem Startwert (-1.7, -1.4) und einer mit dem Startwert (-1.8, -1.4).
...
Dieses System veranschaulicht schön, wie sich eine kleine Variation der Startbedingung auf den System-Verlauf auswirken kann, wobei in diesem speziellen Fall die Lösung mit grösserem $t$ wieder konvergieren würde. Bedenkt man nun, dass eine solch kleine Variation durch stochastisches Rauschen erzeugt werden könnte, scheint es unsinnig eine Lösung für das Problem anzugeben - zumindest nicht ohne auf die Aussagekraft der Lösung hinzuweisen oder sogar den Lösungsraum genauer zu spezifizieren.
...
Um diesem Umstand gerecht zu werden, kann man die Möglichkeit von Zufälligen Störungen beim Aufstellen des Modells miteinbeziehen - et. voilà, man hat eine stochastische Differenzialgleichung (SDGL).

Nun ist die Aussage, also die Lösung der SDGL, nicht mehr ein Zustand zum Zeitpunkt $ t $, sondern eine Wahrscheinlichkeitsverteilung über die verschiedenen möglichen Endzustände unter Einbezug von Störeinflüssen.
...
wobei B eine n x m Matrize mit Faktoren ist, welche beschreiben, wie sich Störungen auf die unterschiedlichen inneren Zustände des Systems auswirken. $ m $ ist demzufolge die Dimension der Störfunktion. In diesem Fall ist die Störung durch sogenanntes $ m $ dimensionales \textit{white noise} ?oder? \glqq{}white noise\glqq{} $\xi(t)$ modelliert.

$ b $ ist der Driftvektor und beschreibt die werwartete Veränderung für jede Dimension des Systems. $ B $ ist die Dispersionsmatrix und beschreibt wies ich das m-dimensionale Rauschen aud das n-dimensionale System auswirkt. Dabei beschreibt die Diagonale der Matrix wie stark sich das Rauschen auf jede Dimension auswirkt und die Dreieckberreiche der Matrix beschreiben die Beeinflussung der Dimensionen untereinander(Korrelation). Alles in allem beschreiben b und B die Dynamik des Systems. 
...
Um das sogenannte \textit{white noise} ???korrekt oder einfach Rauschen??? zu modellieren, muss eine Definition für einen stochastischen Prozess gefunden werden. Zwei zentrale Konzepte sind der \textit{random walk} und der Wienerprozess.
...
Der Wiener-Prozess ist ein kontinuierlicher stochastischer Prozess, er kann als Grenzwert eines Random Walks betrachtet werden, wenn die Zeitschritte gegen null gehen und die Schrittlängen normalverteilt sind. Speziell dabei ist, dass unabhängig von der verstrichenen Zeit der Erwartungswert dem Ausgangswert entspricht. Er muss also folgende Eigenschaften erfüllen: 
