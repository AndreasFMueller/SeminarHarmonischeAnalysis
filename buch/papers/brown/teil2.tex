...
\begin{definition}
{\em Weisses Rauschen:}
Ein zufälliges Signal mit einer konstanten spektralen Leistungsdichte über alle Frequenzen hinweg. Dies bedeutet, dass alle Frequenzkomponenten mit gleicher Intensität vertreten sind. Das Rauschen wird als "weiß" bezeichnet, analog zu weissem Licht, bei dem alle Farben des sichtbaren Spektrums überlagert sind, wobei anzumerken ist, dass weisses Licht keine konstante Leistungsdichte im Spektrum aufweisst. In der Zeitdomäne ist das weisse Rauschen ein unkorreliertes Signal.
\end{definition}

Ein Beispiel dafür ist das Rauschen von Radios, wenn die Frequenz nicht korrekt eingestellt ist, ausgelöst durch Störeinflüsse wie zum Beispiel: Elektrische Interferenzen, kosmische Einflüsse oder auch Blitzentladungen.


\begin{definition}
	{\em Rosa Rauschen:}
	Die Amplituden des Rauschens werden mit zunehmender Frequenz kleiner, weisen also einen invers proportionalen Zusammenhang auf $ ~1/f $. Der Begriff ``Rosa'' ist eine Analogie zum sichtbaren Licht, bei dem tiefere Frequenzen auch eher rötlich erscheinen und überlagert mit Weiss (weisses Rauschen), Rosa ergeben. In der Elektronik wird der Abfall der Leistungsdichte mit 3 \text{dB} pro Oktave charakterisiert.
\end{definition}

Um bei einem hörbaren Beispiel zu bleiben: Rosa Rauschen klingt als Schall ausgewogener und ``weicher'' als weißes Rauschen. Dies, da unangenehme hohe Frequenzen abgeschwächt werden und somit weniger dominant sind.

\begin{definition}
	{\em Braunes Rauschen (Brownsches Rauschen):}
	Die Leistungsdichte nimmt im Spektrum mit zunehmender Frequenz invers quadratisch ab ($ ~1/f^2 $). \textit{Baun} bezieht sich hier nicht auf eine Farbe, sondern ist Robert Brown gewidmet. Denn die Brownsche Modekühlbewegung entspricht diesem Rausch-Typ, da sich die beobachteten trägen Moleküle mit zunehmender Frequenz verstärkt gegenseitig behindern. In der Elektronik wird der Abfall der Leistungsdichte mit 6 \textbf{dB} pro Oktave charakterisiert.
\end{definition}

\begin{definition}
	{\em Gaussisches Rauschen:}
	Die Amplituden im Frequenzspektrum weisen eine Normalverteilung (Gaussverteilung) um eine zentrale Frequenz auf.
\end{definition}
...
\begin{definition}
	{\em Weisses gaußsches Rauschen:}
	Ein zufälliges Signal, bei dem die Amplituden der Rauschwerte zu jedem gegebenen Zeitpunkt einer Normalverteilung (Gaussverteilung) folgen. Das bedeutet, dass die Wahrscheinlichkeit eines bestimmten Rauschwerts (Amplitude) durch die Gleichung der Normalverteilung bestimmt wird. Gaussisches Rauschen ist durch zwei Parameter charakterisiert: den Erwartungswert (Mittelwert) und die Standardabweichung (Varianz) der Verteilung.
\end{definition}

Gaussisches Rauschen ist ein häufiges Artefakt in digitalen Bildern und daher von besonderer Bedeutung in der digitalen Bildverarbeitung. Die charakteristische "Körnigkeit" des Rauschens in einem Bild kann durch seine statistischen Eigenschaften, wie die Varianz und und Mittelwert der räumlichen Frequenz, beschrieben werden.

\begin{definition}
	{\em Impulsrauschen:}
	Das Signal weist kurzzeitige und unerwartete Spitzen oder Abfälle in der Signalintensität auf, also diskontinuierliche, skalierte Impulse. Im Frequenzspektrum äussern sich die Impulse über einen breiten Bereich. Das Konzept hier ist ähnlich zur Fourier-Transformation eines Dirac-Impulses. Ein idealer Dirac-Impuls im Zeitbereich transformiert zu einem konstanten Wert über alle Frequenzen im Frequenzbereich.
\end{definition}

In der digitalen Bildverarbeitung ist diese Art von Rauschen als \textit{salt \& pepper} Rauschen bekannt. Dabei nehmen einzelne Pixel sporadisch extreme Werte an, entweder extrem dunkel (pepper) oder extrem hell (salt). Diese Pixel sind auf dem Bild leicht erkennbar und erscheinen isoliert, ähnlich wie verstreute Salz- oder Pfefferkörner.
...
Der Wiener-Prozess und die brownsche Bewegung werden oft als synonym verwendet. Beides sind kontinuierliche stochastische Prozesse, wobei der Wiener-Prozess explizit nirgends differenzierbar ist und eher in der Mathematik zur Anwendung kommt.
...
\begin{equation}
	\xi(t) = \frac{dW(t)}{dt}
\end{equation}
ergibt weisses Rauschen. Dies ist auch der Grund, wesshalb der Wiener-Prozess als nicht differenzierbar definiert ist. Die Änderungen sind rein zufällig und normalverteilt, so ergibt sich ein gleichmässiges Frequenzspektrum der Änderungsraten. Es ist also keinerlei Information in den Änderungsraten enthalten.
