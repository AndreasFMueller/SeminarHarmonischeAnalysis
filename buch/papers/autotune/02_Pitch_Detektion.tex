...
Es gilt nun diese grundharmonische Frequenz zu finden welche als dominate Tonhöhe wahrgenommen wird.
Es gibt mehrere Möglichkeiten dies umzusetzen.
Im Rahmen dieses Papers werden zwei Methoden verglichen.

Grundsätzlich gillt, je genauer die Tonhöhe bestimmt werden kann, desto besser kann die Tonhöhe anschliessend korrigiert werden.
Die spektrale Frequensauflösung ist ein direktes Mass für die Genauigkeit der Tonhöhenbestimmung.
...
\begin{equation}
    f_{\text{pitch}}
    =
    \arg\max_f \left| \; \mathbf{STFT}_x(m, w)[f] \; \right|^2.
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Die $\mathbf{CMNDF}$ liefert den normalisierten absoluten \glqq Überlagerungsfehler\grqq\ zum Zeitpunkt $t$ des Signals $f$ und einer um $\tau$ verzögerten Version von $f$.
...
Dies ist im Beispiel eines realen Signals (Aufnahme Gesang) in Abbildung \ref{autotune:fig:cmndfRealSignal} ersichtlich.
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{papers/autotune/images/Example-CMNDF.pdf}
	\caption{CMNDF eines realen Signals (Aufnahme Gesang) mit mehreren lokalen Minima.}
