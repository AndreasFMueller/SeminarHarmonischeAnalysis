...
Die in \eqref{opt:equation:integral_fraunhofer} hergeleitete elektrische Feldstärke kann leider nicht direkt beobachtet werden.
Aufgrund dessen wird im folgenden ein möglicher Weg dargestellt, den Zusammenhang zwischen der elektrischen Feldstärke $E$ und der Intensität $I$ zu finden.
Diese Herangehensweise gliedert sich wiederum in drei Abschnitte.
Zuerst wird im Abschnitt \ref{opt:subsection:proportionality} ...
\opttodo{Text hier hinzufügen}

\subsection{Proportionalität}
\label{opt:subsection:proportionality}
...
Andere Eigenschaften wie die Phase des Lichtes können von diesen Beobachtern nicht unterschieden werden.

Ein wenig aus der Luft gegriffen definieren wir vorerst, dass sich die Intensität $I$ proportional zur quadrierten elektrischen Feldstärke verhält.
Dass sich diese Annahme aber tatsächlich bewahrheitet, wird sich schliesslich in \ref{opt:subsection:wave_impedance} ergeben.
Als Proportionalitätskonstante wird in diesem Paper $\kappa$ verwendet.
...
Allerdings ist immer noch unklar, was für einen Wert $\kappa$ wirklich annimmt.
Der Poynting Vektor soll hierbei Aushilfe schaffen.
...
Soll nun der exakte Wert von $\kappa$ bestimmt werden, werden weitere Kenntnisse der Elektrodynamik benötigt.
Dieser Wert kann mittels des Poynting Vektors
\begin{equation}
\vec{S} = \vec{E} \times \vec{H}
\label{opt:equation:poynting}
\end{equation}
hergeleitet werden.
Der Poynting Vektor ist ebenso wie die Intensität ein Mass für die Energie pro Fläche.
...
Da das zweite Moment des Real- oder Imaginärteils des Poynting Vectors gerade der gesuchten Intensität $I$ entspricht, kann diese als 
\begin{align*}
I
&=
E(\Re^2(S))
...
Um ihn entsprechend auf Gleichung \eqref{opt:equation:intensity_integral} umformen zu können, muss der Begriff der Wellenimpedanz eingeführt werden.
...
Dieses Verhältnis von $E_0$ zu $H_0$ wird auch als Wellenimpedanz bezeichnet und spielt in der Elektrodynamik eine essentielle Rolle.
...
\end{equation*}
entsprechen muss.
\opttodo{Bezug auf \ref{opt:subsection:proportionality} herstellen}
